...
def save_execution_history(executor_email, start_date, end_date, settlement_date, num_plants, reason):
    history_file = "settlement_history.csv"
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    new_data = pd.DataFrame([{
        "실행 일시": now,
        "실행자 이메일": executor_email,
        "정산 시작일": start_date,
        "정산 종료일": end_date,
        "수정정산일(yymmdd)": settlement_date,
        "처리된 총 발전소 수": num_plants,
        "수정 사유": reason,
        "진행 상태": "성공"
    }])
    
    if os.path.exists(history_file):
        new_data.to_csv(history_file, mode='a', header=False, index=False, encoding='utf-8-sig')
    else:
        new_data.to_csv(history_file, mode='w', header=True, index=False, encoding='utf-8-sig')
...
def main():
    ...
    tab1, tab2 = st.tabs(["수정정산 실행", "📜 실행 기록 조회"])
    ...
    with tab1:
        with st.form("settlement_form"):
            ...
            submitted = st.form_submit_button("1단계: 정산서 파일 생성 및 업로드 📁")
        ...
        if submitted:
            ...
            try:
                ...
                status.text("✅ 1단계: 파일 생성 및 업로드 완료!")
                st.success("모든 발전소에 대한 산출물 생성 및 구글 드라이브 업로드를 완료했습니다. 2단계를 진행해주세요.")
                
                st.session_state['generated_tasks'] = generated_tasks
                st.session_state['settlement_meta'] = {
                ...
                st.session_state['executor_email'] = user_email
        ...
        if st.session_state.get('generated_tasks'):
            st.subheader("발전소별 생성 결과 요약")
            df_tasks = pd.DataFrame(st.session_state['generated_tasks'])
            # 깔끔하게 테이블로 보여주기 (필요한 컬럼만)
            st.table(df_tasks[['plant_name', 'recipient']].rename(columns={'plant_name': '발전소명', 'recipient': '수신자 메일'}))
            
            if st.button("2단계: 최종 검증 완료 및 메일 발송 🚀"):
                progress_mail = st.progress(0)
                status_mail = st.empty()
                
                tasks = st.session_state['generated_tasks']
                meta = st.session_state['settlement_meta']
                mail_engine = AutomationMailEngine(google_creds)
                
                total = len(tasks)
                try:
                    for i, task in enumerate(tasks):
                        status_mail.text(f"[{task['plant_name']}] Gmail 전송 중...")
                        mail_engine.send_settlement_email(
                            sender=meta['sender_email'],
                            recipient=task['recipient'],
                            cc=meta['cc_email'],
                            plant_name=task['plant_name'],
                            period_str=meta['period_str'],
                            reason=meta['reason'],
                            attachments=[task['pdf_path'], task['excel_path']]
                        )
                        progress_mail.progress(int(100 * (i + 1) / total))
                    
                    status_mail.text("✅ 메일 발송 완료!")
                    st.success("모든 메일 발송이 성공적으로 완료되었습니다.")
                    
                    # 히스토리 저장
                    save_execution_history(
                        executor_email=st.session_state['executor_email'],
                    ...
                    del st.session_state['settlement_meta']
                    st.rerun() # 화면 갱신
                    
                except Exception as e:
                    st.error(f"❌ 메일 발송 중 에러 발생: {str(e)}")
                    with st.expander("에러 상세 로그 확인"):
                        st.code(traceback.format_exc())

    with tab2:
        st.subheader("📜 실행 기록 조회")
        history_file = "settlement_history.csv"
        if os.path.exists(history_file):
            try:
                df_history = pd.read_csv(history_file)
                # 최신순 정렬 (실행 일시 내림차순)
                df_history = df_history.sort_values(by="실행 일시", ascending=False).reset_index(drop=True)
                st.dataframe(df_history, use_container_width=True)
            except Exception as e:
                st.error("실행 기록을 불러오는 중 오류가 발생했습니다.")
                st.error(str(e))
        else:
